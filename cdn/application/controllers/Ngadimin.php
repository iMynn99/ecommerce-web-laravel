...
class Ngadimin extends CI_Controller {
	...
	*/
	...
	public function laporanproduk(){
		...
		if(isset($_GET["load"])){
			$res = $this->load->view('admin/laporanproduklist',"",true);
			echo json_encode(["result"=>$res,"token"=>$this->security->get_csrf_hash()]);
		}else{
			$this->load->view('admin/head',["menu"=>19]);
			$this->load->view('admin/laporanproduk');
			$this->load->view('admin/foot');
		}
	}
	...
	public function laporankomisi(){
		...
		if(isset($_GET["load"])){
			$res = $this->load->view('admin/laporankomisilist',"",true);
			echo json_encode(["result"=>$res,"token"=>$this->security->get_csrf_hash()]);
		}else{
			$this->load->view('admin/head',["menu"=>26]);
			$this->load->view('admin/laporankomisi');
			$this->load->view('admin/foot');
		}
	}
	...
	public function paketkurir(){
		...
		$this->load->view('admin/paketkurir');
		$this->load->view('admin/foot');
	}
	...
	public function broadcast(){
		...
		$this->load->view('admin/head',["menu"=>25]);
	...
	public function topupsaldo(){
		...
		$this->load->view('admin/saldotopup');
		$this->load->view('admin/foot');
	}
	public function tariksaldo(){
		...
		$this->load->view('admin/head',["menu"=>23]);
		$this->load->view('admin/saldotarik');
		$this->load->view('admin/foot');
	}
	...
	public function nonmember(){
		...
		$this->load->view('admin/head',["menu"=>27]);
		$this->load->view('admin/nonmember');
		$this->load->view('admin/foot');
	}
	...
	function hapuskategori(){
		...
		if(isset($_POST["pro"])){
			...
			foreach($dbs->result() as $R){
				if(file_exists("kategori/".$R->icon)){
