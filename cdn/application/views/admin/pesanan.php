...
<script type="text/javascript">
	...
	function cetak(inv){
		window.open("<?=site_url("api/cetakInvoice")?>"+"?id="+inv);
	}
	...
	function lacakPaket(orderid){
		...
		$("#lacakloader").show();
		$("#lacakload").hide();
		$("#modalacak #lacakload").load("<?=site_url('api/lacakiriman')?>?orderid="+orderid,function(){
			$("#lacakloader").hide();
			$("#lacakload").show();
		});
	}
	function selesai(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("api/terimapesanan")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
					updateToken(data.token);
	...
	function batalkan(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("api/batalkanpesanan/bymin")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
					updateToken(data.token);
					if(data.success == true){
						loadBatal(1);
