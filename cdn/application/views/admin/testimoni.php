...
<script type="text/javascript">
		...
		$("#sbforms").on("submit",function(e){
			...
			swal.fire({
				...
				if(vals.value){
					...
					$.ajax({
						url        : '<?php echo site_url("api/tambahtestimoni"); ?>',
						type       : 'POST',
						contentType: false,
						cache      : false,
						processData: false,
	...
	function edit(id){
		$.post("<?=site_url('api/testimoni')?>",{"formid":id,[$("#names").val()]:$("#tokens").val()},function(ev){
			var data = eval("("+ev+")");
			updateToken(data.token);
			$("#id").val(id);
			$("#nama").val(data.nama);
	...
	function hapus(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("api/hapustestimoni")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
					updateToken(data.token);
