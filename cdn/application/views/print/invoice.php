	...
	<div class="nota">
		<?php
			...
			foreach($db->result() as $trx){
				...
				$user = ($trx->usrid > 0) ? $this->func->getUserdata($trx->usrid,"semua") : $this->func->getUserTemp($trx->usrid_temp,"semua");
				...
				<table class="table">
					...
					<?php
						...
						foreach($db->result() as $r){
							...
							$variasi = (is_object($vari)) ? $prod->variasi." ".$this->func->getVariasiWarna($vari->warna,"nama") : "";
							$variasi .= (is_object($vari) AND ($vari->size > 0)) ? ", ".$prod->subvariasi." ".$this->func->getVariasiSize($vari->size,"nama") : "";
							echo "
								<tr>
									<td>".$kode."</td>
